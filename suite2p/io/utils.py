...
def get_bin_list(ops): #ES_edit
    """ make list of bins to process
    if ops['subfolders'], then all bins ops['data_path'][0] / ops['subfolders'] / *.bin
    if ops['look_one_level_down'], then all bin in all folders + one level down
    if ops['bin_list'], then ops['data_path'][0] / ops['bin_list'] ONLY
    """
    froot = ops['data_path']
    # use a user-specified list of tiffs
    if 'bin_list' in ops:
        fsall = []
        for tif in ops['bin_list']:
            fsall.append(os.path.join(froot[0], tif))
        ops['first_bins'] = np.zeros((len(fsall),), dtype='bool')
        ops['first_bins'][0] = True
        print('** Found %d bins**'%(len(fsall)))
    else:
        if len(froot)==1:
            if 'subfolders' in ops and len(ops['subfolders'])>0:
                fold_list = []
        ...
        first_bins = []
        for k,fld in enumerate(fold_list):
            fs, fbins = list_files(fld, ops['look_one_level_down'],
                                    ["*.bin"])
            fsall.extend(fs)
            first_bins.extend(list(fbins))
        if len(fsall)==0:
            print('Could not find any bins')
            raise Exception('no bins')
        else:
            ops['first_bins'] = np.array(first_bins).astype('bool')
            print('** Found %d bins**'%(len(fsall)))
    return fsall, ops
...
def find_files_open_binaries(ops1, ish5=False):
    ...
    elif input_format == 'bin': #ES_edit
        fs, ops2 = get_bin_list(ops1[0])
        print('Bin files')
        print(fs)
        for ops in ops1:
            ops['first_bins'] = ops2['first_bins']
            ops['frames_per_folder'] = np.zeros((ops2['first_bins'].sum(),), np.int32)
