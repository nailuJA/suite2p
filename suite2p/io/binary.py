...
class BinaryRWFile:
    ...
    def data(self) -> np.ndarray:
        ...
        """
        with temporary_pointer(self.file) as f:
            binf = np.fromfile(f, np.int16)
            binfshape = binf.shape[0]
            binobjshape = self.n_frames * self.Ly * self.Lx
            
            if binfshape != binobjshape: #ES_edit
                print(f'BinaryRWFile.Data assumes {binfshape} data_points while '\
                      f'BinaryRWFile object assumes {binobjshape}')
                if binfshape == binobjshape + 2:
                    print('Most likely bin file from RawDataStream, ignoring first two datapoints')
                    binf = binf[2:]

            return binf.reshape(-1, self.Ly, self.Lx)
