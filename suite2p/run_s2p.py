...
def pipeline(f_reg, f_raw=None, f_reg_chan2=None, f_raw_chan2=None, 
    ...
    if ops.get('roidetect', True):
        ...
        t11=time.time()
        print('----------- EXTRACTION')
        if 'mask_path' in ops.keys(): #ES_edit
            stat = np.load(os.path.join(ops['mask_path'], 'stat.npy'), allow_pickle = True) #ES_edit
            print("Overwriting stat with stat file from ops['mask_path']")
...
def run_s2p(ops={}, db={}, server={}):
    ...
    # if not set up files and copy tiffs/h5py to binary
        ...
        elif ops.get('already_bin'): #ES_edit
            ops['input_format'] = 'bin'
    ...
    if ops.get('multiplane_parallel'):
        ...
        if 'mask_path' in ops.keys():#ES_edit
            shutil.copy(os.path.join(ops['mask_path'], 'iscell.npy'), os.path.join(op['save_path'], 'iscell.npy'))
